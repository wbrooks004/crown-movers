...
| `10-archive-service.json` | **clipboard, not a template** | paste onto a designed Page — dynamic query loop over `page` posts tagged with any `service_category` term. See **Architecture correction**. |
| `11-archive-location.json` | **clipboard, not a template** | paste onto a designed Page — dynamic query loop over the `location` CPT, own `location-card__*` class family. See **Architecture correction**. |
...
| `14-archive-blog.json` | template (archive) | Bricks → Templates → Import — native WP `post` type |
...
Fixed in this pass:
- `10-archive-service.json` and `11-archive-location.json` are no longer Bricks *templates* — they're clipboard sections meant to be pasted onto a normal, human-created Page (their `{archive_title}`/`{archive_description}` tags, which only resolve on a real archive route, were swapped for `{post_title}` and a hand-typed intro, matching how every other paste-once page in this pack works).
...
| Services listing | `10-archive-service.json` | Yes — paste onto a Page; not a CPT archive, see **Architecture correction** |
| Single service | `12-single-service.json` | Yes — real Page fields, targets `service_category::all` |
| Locations listing | `11-archive-location.json` | Yes — paste onto a Page; not a CPT archive |
| Single location | `13-single-location.json` | Yes — real `location` CPT fields |
| Blog archive / Single post | `14`, `15` | Yes — native `post` type |
